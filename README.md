...
初回 apply では DNS 委譲前でも止まらないように、`dns_delegation_completed=false` のまま実行します。
...
CloudFront とエイリアスレコードは、NS 委譲完了後の 2 回目 apply で作成されます。
...
NS 委譲が完了したら、`dns_delegation_completed=true` を付けて再度 apply します。

```bash
terraform apply -var dns_delegation_completed=true
```

GitHub Actions では、リポジトリ変数 `DNS_DELEGATION_COMPLETED` を `true` にすると同じ切り替えが反映されます。
